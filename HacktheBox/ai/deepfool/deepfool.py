...
def png_from_x_anysize(x01: np.ndarray, size: tuple[int, int]) -> str:
    ...
    """
    x255 = np.clip((x01 * 255.0).round(), 0, 255).astype(np.uint8)
    img = Image.fromarray(x255, mode="L").resize(
...
def deepfool_targeted(
    ...
    with torch.enable_grad():
        for _ in range(max_iter):
            ...
            # Gradient for current prediction
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(downloads from /weights if missing)"
        ),
    )
    ...
    if clean_pred != chall.label:
        print(
            f"Warning: local clean prediction {clean_pred} "
            f"!= label {chall.label}; proceeding regardless"
        )

    # Run targeted DeepFool with a robust overshoot schedule
    # against PNG quantization
    ...
    for ov in overshoots:
        ...
        cand_l2 = float(
            np.linalg.norm(cand - chall.x01)
        )

        if (
            cand_pred == chall.target
            and cand_l2 <= chall.l2_threshold
        ):
            x_adv = cand
            adv_pred = cand_pred
            l2 = cand_l2
    ...
    if x_adv is None:
        # Take the closest candidate and proceed;
    ...
    # 2) Too large L2 (zeros image),
    # likely misclassified but exceeds threshold
    ...
    # 7) Adversarial but wrong target
    # (should fail targeted check while within L2)
    def craft_wrong_target() -> dict | None:
        # Try all non-target classes and several overshoots;
        # pick first that meets L2 and predicts that class.
        classes = [
            c
            for c in range(10)
            if c != chall.target
